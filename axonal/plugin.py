...
def _modname(cls, full=False):
    ...
    if full and module == "__main__":
        import inspect
        spec = inspect.getmodule(cls).__spec__
...
def _fullname(obj):
    ...
    if obj is None:
        return None
    return _modname(obj, True) + '.' + obj.__class__.__name__
...
class Host(Plugin):
    ...
    def options(self, parser, env):
        ...
        parser.add_argument(
            '-0', '--name', metavar="code", dest='name',
            default=plugin_name, help='Process name')
        parser.add_argument(
            '-L', '--log-config', metavar="filename", dest='logconfig',
            default=env.get('LOGGING_CONF'),
            type=argparse.FileType('r'), help='Logging configuration file')
        parser.add_argument(
    ...
    def configure(self, options, conf):
        ...
        try:
            ...
            self._log.warning("Process name unchanged")
...
class Loader(Plugin):
    ...
    def run(self):
        Host(self._plugin).main(self._args)
